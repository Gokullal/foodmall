...
if(isset($_POST['submit'])){
    ...
    $sql="SELECT * FROM shopreg WHERE (email='$username' OR phone='$username') AND password='$password' AND status=1 ";
    $result = mysqli_query($conn,$sql);
    if(mysqli_num_rows($result)>0){
        $_SESSION["username"] = $username;
        $_SESSION["password"] = $password;
        header("Location: http://localhost/foodmall/components/shoplandingpage.php");
    }
    else{
        echo '<script>alert("Either check the password or wait for admin to approve")</script>';
    }
}
?>
...
<html>
    <head>
        ...
        <link type="text/css" rel="stylesheet" href="http://localhost/foodmall/css/login.css">
        <script>
    ...
    <div id="login-container">
            <div id="login">
                ...
                <form action="" method="post" id="login-form" name="myForm" onsubmit="return validateForm()">
                    ...
                    <p id="register-now">Not a user?<a href="http://localhost/zebodo/components/shopreg.php" id="register-link">Register Now</a></p>
